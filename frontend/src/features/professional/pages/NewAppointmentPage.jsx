...
import { agendaKeys, useAssignedAppointments } from '../hooks/useAgenda';
...
export function NewAppointmentPage() {
  ...
  const initialDate = params.get('fecha') || today;
  const initialDayId = params.get('diaAgendaId');
  const origin = params.get('origen');
  const initial = parseDateString(initialDate);
  const [step, setStep] = useState(initialDayId ? 2 : 1);
  const [query, setQuery] = useState('');
  const [clients, setClients] = useState([]);
  const [client, setClient] = useState(null);
  const [days, setDays] = useState([]);
  const [candidateDay, setCandidateDay] = useState(null);
  const [selectedDay, setSelectedDay] = useState(() => initialDayId ? { id: initialDayId, diaAgendaId: initialDayId, fecha: initialDate, seleccionable: true } : null);
  ...
  useEffect(() => {
    ...
    setBusy(true);
    getAppointmentDay(initialDayId)
      .then((data) => {
        setDayDetail(data);
        setSelectedDay((current) => ({ ...current, ...data, id: data.id ?? data.diaAgendaId ?? initialDayId, diaAgendaId: data.diaAgendaId ?? data.id ?? initialDayId, fecha: data.fecha || initialDate, seleccionable: true }));
      })
      .catch((err) => {
        showError('No se pudo cargar el día seleccionado', err.message);
        setStep(1);
      })
      .finally(() => setBusy(false));
  }, [initialDate, initialDayId, showError]);
  ...
  async function validate() {
    setBusy(true);
    try { setPreview(await validateManualAppointment(payload)); setStep(3); }
  ...
  async function confirm() {
    ...
    try {
      ...
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: agendaKeys.assignedAppointments(date, date) }),
  ...
  return <div className="space-y-6">
    <PageHeader
      eyebrow="Operación"
      title="Nuevo turno"
      description="Alta manual de un turno para un cliente"
      actions={origin === 'mi-dia'
        ? <Button variant="outline" onClick={() => navigate(`/profesional/mi-dia?fecha=${initialDate}`)}><ChevronLeft />Volver al día</Button>
        : origin === 'mi-mes'
          ? <Button variant="outline" onClick={() => navigate(`/profesional/mi-mes?mes=${initialDate.slice(0, 7)}`)}><ChevronLeft />Volver al mes</Button>
          : null}
    />
    <div aria-label="Progreso del alta" className="grid gap-2 sm:grid-cols-3">
      {['Día', 'Cliente, horario y atención', 'Confirmación'].map((label, index) => {
        const target = index + 1;
        return target < step
          ? <button type="button" key={label} className="rounded-lg border px-3 py-2 text-left text-sm transition hover:bg-muted" onClick={() => { if (target < 3) setPreview(null); setStep(target); }}>{target}. {label}</button>
    ...
    {step === 1 && <Card><CardHeader><div className="flex items-center justify-between gap-3"><div><CardTitle>Seleccionar día</CardTitle><p className="mt-1 text-sm text-muted-foreground">Un clic marca el día; doble clic lo selecciona directamente.</p></div><div className="flex items-center gap-1"><Button variant="outline" size="icon" aria-label="Mes anterior" onClick={() => changeMonth(-1)}><ChevronLeft /></Button><MonthWheelPicker aria-label="Seleccionar mes del turno" value={viewMonth.month} onChange={(month) => { setViewMonth((current) => ({ ...current, month })); setCandidateDay(null); }} triggerVariant="ios-compact" /><Button variant="outline" size="icon" aria-label="Mes siguiente" onClick={() => changeMonth(1)}><ChevronRight /></Button></div></div></CardHeader><CardContent className="space-y-4">
      ...
      <div className="flex justify-end"><Button disabled={!candidateDay?.seleccionable || busy} onClick={() => acceptDay()}>Seleccionar día</Button></div>
    </CardContent></Card>}

    {step === 2 && <Card><CardHeader><div className="flex items-center justify-between"><CardTitle>Cliente, horario y atención</CardTitle><Button variant="outline" onClick={() => { setPreview(null); setStep(1); }}><ChevronLeft />Cambiar día</Button></div></CardHeader><CardContent className="grid gap-6 lg:grid-cols-3">
      ...
      <section className="space-y-4 rounded-xl border bg-muted/15 p-4 lg:col-span-2" aria-label="Horario estimado">
        <div className="space-y-4 border-b pb-4" aria-label="Seleccionar cliente">
          <div><p className="font-semibold">Seleccionar cliente</p><p className="text-sm text-muted-foreground">Buscá por nombre, apellido o DNI antes de completar el horario.</p></div>
          ...
          <div className="grid gap-2 sm:grid-cols-2">{clients.map((item) => <Button key={item.id} variant={client?.id === item.id ? 'default' : 'outline'} className="h-auto justify-start py-3" onClick={() => { setClient(item); setPreview(null); }}>{item.nombre} {item.apellido} · {item.tipoDocumento} {item.numeroDocumento}</Button>)}</div>
        </div>
        ...
        <Button disabled={!client || !typeId || !start || !end || start >= end || busy} onClick={validate}>Revisar avisos y confirmar</Button>
      </section>
    </CardContent></Card>}

    {step === 3 && preview && <Card><CardHeader><CardTitle>Confirmar turno</CardTitle></CardHeader><CardContent className="space-y-4">
      ...
      <div className="flex gap-2"><Button aria-label="Volver al horario" variant="outline" onClick={() => { setPreview(null); setStep(2); }}><ChevronLeft />Modificar</Button><Button onClick={confirm} disabled={busy}><CalendarPlus />{busy ? 'Creando...' : 'Confirmar y crear'}</Button></div>
    </CardContent></Card>}
  </div>;
}
